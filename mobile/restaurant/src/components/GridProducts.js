...
const ProductItem = (props) => {
  ...
  return (
    ...
    >
      {/* {console.log(`http://192.168.0.102:19001/assetsTest/${props.img}`)} */}

      <Image
        src={`data:image/png;base64,${props.img}`}
        // style={{ width: "50%", height: "50%" }}
        // source={props.img}
        // source={{ uri: `http://192.168.0.102:19001/assetsTest/${props.img}` }}
        source={{ uri: `http://localhost:3000/meniul-zilei` }}
        style={{
        ...
        resizeMode="contain"
      />
      <Text style={styles.name}>{props.denumire}</Text>
      {/* <Text style={styles.type}>{props.categoryName}</Text> */}
      <Text style={styles.type}>{props.descriere} g</Text>
      <View style={styles.bottom}>
        <Text style={styles.priceView}>
          <Text style={styles.price}>{props.pret} </Text>

          <Text> lei</Text>
          {/* <Text>Stoc portii {props.stoc_portii}</Text> */}
        </Text>

        <TouchableOpacity
          onPress={() => {
            addProductToCard(props);
            Alert.alert(`Am adăugat în coș ${props.denumire}!`);
          }}
        >
